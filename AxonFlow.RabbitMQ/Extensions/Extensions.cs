...
namespace AxonFlow
{
  ...
  {
    /// <summary>
    /// Add the AxonFlow.Router.Router RabbitMQ message dispatcher to the service collection, allowing it to be resolved and used.
    /// </summary>
      ...
      Action<MessageDispatcherOptions> config)
    {
      ...
      services.AddKeyedSingleton<IExternalMessageDispatcher, MessageDispatcher>(global::AxonFlow.Router.RouterKeyServicesName);
      return services;
    }
    ...
    public static RouterOptions NotificationsInASingleQueue(this RouterOptions options, Func<IEnumerable<Type>, IEnumerable<Type>> notificationTypes = null)
    {
      ...
      {
        options.SetTypeQueueName(n, $"{n.AxonTypeName(options)}${Assembly.GetEntryAssembly()?.GetName().Name.Replace(".", "_")}");
      }

      return options;
    }

    public static RouterOptions NotificationsInASingleQueueBut(this RouterOptions options, Func<IEnumerable<Type>, IEnumerable<Type>> notificationTypes = null)
    {
      var notifications = options.LocalTypes.Where(t => t.IsNotification());
