...
using Constants = AxonFlow.RabbitMQ.Constants;
...
namespace AxonFlow.RabbitMQ
{
  ...
  public class RequestsManager : IHostedService
  {
    ...
    private async Task CheckConnection(CancellationToken cancellationToken)
    {
      ...
      if (_connection == null)
      {
        _logger.LogInformation($"AxonFlow: Creating RabbitMQ Connection to '{_options.HostName}'...");
        ...
        _logger.LogInformation("AxonFlow: ready !");
      }
    }
