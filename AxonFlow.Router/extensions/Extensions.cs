...
{
  /// <summary>
  /// Extension methods for configuring and using AxonFlow.Router in an ASP.NET Core application.
  /// </summary>
  ...
  {
    /// <summary>
    /// Adds the AxonFlow.Router to the service collection.
    /// </summary>
    ...
    public static IServiceCollection AddAxonFlow(this IServiceCollection services, Action<RouterOptions> configure = null)
    {
      ...
      services.AddSingleton<IRouter, global::AxonFlow.Router>();

      services.AddTransient<global::AxonFlow.IAxon, AxonFlow>();
      return services;
    }

    /// <summary>
    /// Adds the AxonFlow.Router service to the specified <see cref="ServiceCollection"/>.
    /// </summary>
    /// <param name="services">The <see cref="ServiceCollection"/> to add the AxonFlow.Router service to.</param>
      ...
      ILogger logger = null)
    {
      var localRequests = assemblies.SelectMany(a => a
        ...
        .SelectMany(t => t.GetInterfaces()
          .Where(i => i.FullName != null && i.FullName.StartsWith("AxonFlow.IRequestHandler"))
      ...
      ILogger logger = null)
    {
      var localNotifications = assemblies.SelectMany(a => a
        .GetTypes()
        .SelectMany(t => t.GetInterfaces()
          .Where(i => i.FullName != null && i.FullName.StartsWith("AxonFlow.INotificationHandler"))
          .Select(i => i.GetGenericArguments()[0]).ToArray()
        ));
